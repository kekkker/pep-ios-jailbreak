...
private final class NativeNotifier {
    ...
    private func acquireMailEngineOwnership() {
        ...
        do {
            ...
            log("unable to create notification queue: \(error.localizedDescription)")
            exit(74)
        }
    ...
    private func queueBulletin(sender: String?, subject: String?) {
        ...
        queue.async { [weak self] in
            do {
                ...
                log("queued pEp notification payload")
            } catch {
                log("unable to queue notification: \(error.localizedDescription)")
            }
        }
    }
}
